...
<style>
  ...
  <article>
    ...
    <div class="template">
      <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$jase): $mod = ($i % 2 );++$i;?><h3>
        <p><span><?php echo ($jase["designation"]); ?></span></p>
        <a href="<?php echo U(Home.'/Photo/more',array('id'=>$jase['id']));?>" class="more">更多>></a> </h3>
      <ul>
        <?php if(is_array($jase["son"])): $i = 0; $__LIST__ = $jase["son"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$jaseson): $mod = ($i % 2 );++$i;?><li><a href="<?php echo U(Home.'/Photo/details',array('id'=>$jaseson['id']));?>"  target="_blank"><img src="/guest2/Uploads/Photo/<?php echo ($jaseson["pic"]); ?>"></a><span><?php echo ($jaseson["title"]); ?></span></li><?php endforeach; endif; else: echo "" ;endif; ?>
